...
export const addUpdateAboutUs =async (req:Request, res:Response) => {
    try {
        ...
        const { companyName, year, business, aboutUsDetail, image, document } = req.body;
...
export const getAboutUs =async (req:Request, res:Response) => {
    try {
        ...
        const sql = `SELECT id, company_name, business, year, about_detail, images, document, created_at FROM about WHERE user_id = ${userId}`;
