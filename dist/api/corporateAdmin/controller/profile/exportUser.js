...
const exportUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        let columnsHeader = [
            ...
            { key: 'display_email', header: 'Email' },
            { key: 'display_dial_code', header: 'Email' },
            { key: 'display_number', header: 'Phone Number' },
            ...
            for (var _l = true, parsedData_1 = __asyncValues(parsedData), parsedData_1_1; parsedData_1_1 = yield parsedData_1.next(), _e = parsedData_1_1.done, !_e;) {
                ...
                try {
                    ...
                    if (cardData.length === 0) {
                        parsedData[dataIndex].failedMessage = `Code is invalid, Contact support!`;
                    ...
                    if (dupliRows.length > 0) {
                        ...
                        //     // dupli.push("phone");
                        // }
                        parsedData[dataIndex].failedMessage = `${dupliKey} is already used!!`;
                        duplicateData.push(parsedData[dataIndex]);
                        continue;
                    }
                    const sql = `INSERT INTO users(admin_id, name, full_name, email, display_email, password, username, card_number, dial_code, qr_code, phone, display_dial_code, display_number, country, offer_coin, quick_active_status, is_deactived, is_verify, is_payment, is_active, is_expired, is_card_linked, post_time, start_date, verify_time, login_time, end_date, account_type, primary_profile_slug, primary_profile_link) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
                    const VALUES = [userId, name, name, email, email, hashPasswwoed, username, cardNumber, dial_code, qrData.data, phone, dial_code, phone, country, 100, 1, 0, 1, 1, 1, 0, 1, justDate, justDate, justDate, justDate, endDate, packageId, 'vcard', primaryProfileLink];
                    ...
                    if (rows.affectedRows > 0) {
                        ...
                        parsedData[dataIndex].failedMessage = `Failed to insert`;
                        duplicateData.push(parsedData[dataIndex]);
                        // return apiResponse.errorMessage(re)
                        continue;
                    }
                }
                finally {
