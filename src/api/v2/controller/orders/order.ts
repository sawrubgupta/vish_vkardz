...
const orderResMsg = resMsg.orders.order;
...
console.log("rows", rows);
        ...
        if (rows.length > 0) {
            ...
            let uniqProductIdsArr = [...new Set(productIdsArr.filter((item:any) => item !== null))];
...
export const orderSummary = async (req: Request, res: Response) => {
    try {
        ...
        if (!orderId || orderId === null || orderId === undefined) return apiResponse.errorMessage(res, 400, orderResMsg.orderSummary.invalidOrderId);
